...
test('Pismo Święte oraz czytnik działają', async ({ page }) => {
  ...
  await page.goto('/pismo-swiete')
  await page.goto('/pismo-swiete/joela/3')
  await expect(page.getByRole('heading', { name: 'Rozdział 3' })).toBeVisible()
  await expect(page.locator('.psalm-verses')).toContainText('wyleję Ducha mego')
