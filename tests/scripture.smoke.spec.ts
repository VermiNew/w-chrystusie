...
test('Pismo Święte oraz czytnik działają', async ({ page }) => {
  ...
  await page.goto('/pismo-swiete')
  await page.goto('/pismo-swiete/tobiasza/14')
  await expect(page.getByRole('heading', { name: 'Rozdział 14' })).toBeVisible()
  await expect(page.locator('.psalm-verses')).toContainText('Skończyły się mowy Tobiaszowe')
  await expect(page.getByText('Biblia Jakuba Wujka (1599), wydanie 1839–1840')).toBeVisible()
})
