...
test('Pismo Święte oraz czytnik działają', async ({ page }) => {
  ...
  await page.goto('/pismo-swiete')
  await page.goto('/pismo-swiete/jozuego/24')
  await expect(page.getByRole('heading', { name: 'Rozdział 24' })).toBeVisible()
  await expect(page.locator('.psalm-verses')).toContainText('a ja i dom mój służyć Panu będziemy')
