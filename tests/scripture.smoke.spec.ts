...
test('Pismo Święte oraz czytnik działają', async ({ page }) => {
  ...
  await page.goto('/pismo-swiete')
  await page.goto('/pismo-swiete/2-samuela/12')
  await expect(page.getByRole('heading', { name: 'Rozdział 12' })).toBeVisible()
  await expect(page.locator('.psalm-verses')).toContainText('Tyś jest ten człowiek')
