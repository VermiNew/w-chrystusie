...
test('Pismo Święte oraz czytnik działają', async ({ page }) => {
  ...
  await page.goto('/pismo-swiete')
  await page.goto('/pismo-swiete/1-samuela/17')
  await expect(page.getByRole('heading', { name: 'Rozdział 17' })).toBeVisible()
  await expect(page.locator('.psalm-verses')).toContainText('ja idę do ciebie w imię Pana zastępów')
