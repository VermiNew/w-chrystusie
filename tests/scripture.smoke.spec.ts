...
test('Pismo Święte oraz czytnik działają', async ({ page }) => {
  ...
  await page.goto('/pismo-swiete')
  await page.goto('/pismo-swiete/2-krolewska/2')
  await expect(page.getByRole('heading', { name: 'Rozdział 2' })).toBeVisible()
  await expect(page.locator('.psalm-verses')).toContainText('wstąpił Eliasz przez wicher do nieba')
