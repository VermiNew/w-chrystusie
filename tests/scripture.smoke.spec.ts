...
test('Pismo Święte oraz czytnik działają', async ({ page }) => {
  ...
  await page.goto('/pismo-swiete')
  await page.goto('/pismo-swiete/powtorzonego-prawa/34')
  await expect(page.getByRole('heading', { name: 'Rozdział 34' })).toBeVisible()
  await expect(page.locator('.psalm-verses')).toContainText('nie powstał napotem prorok')
