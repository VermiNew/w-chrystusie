...
test('Pismo Święte oraz czytnik działają', async ({ page }) => {
  ...
  await page.goto('/pismo-swiete')
  await page.goto('/pismo-swiete/2-kronik/7')
  await expect(page.getByRole('heading', { name: 'Rozdział 7' })).toBeVisible()
  await expect(page.locator('.psalm-verses')).toContainText('i uzdrowię ziemię ich')
